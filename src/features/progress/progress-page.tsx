...
import { ClimbingProgress } from "./climbing-progress";
import {
  getProgressSummary,
  logCompletion,
  selectLogsInRange,
...
export function ProgressPage() {
  ...
  const [view, setView] = useState<"nutrition" | "climbing">("nutrition");
    ...
    <>
      <header className="page-heading">
        <div>
          ...
          <h1>Progress</h1>
          <p>Private, on-device trends across nutrition and climbing.</p>
        </div>
      </header>
      <div className="progress-view-switch">
        <SegmentedControl
      ...
      {view === "climbing" ? (
        <ClimbingProgress
          sessions={state.climbingSessions}
          range={range}
          today={today}
        />
      ) : (
        <>
          <dl className="stat-grid" aria-label="Progress summary">
            ...
            <div>
              <dt>Avg. water</dt>
              <dd>
                {summary.averageWater === 0
                  ? "—"
                  : `${(summary.averageWater / 1000).toFixed(1)} L`}
              </dd>
            </div>
            <div>
              <dt>Adherence</dt>
                  ...
                  <article className="history-row" key={log.date}>
                    ...
                    <dl>
                      ...
                      <div>
                        ...
                        <dd>
                          {waterTotal(log)
                            ? `${(waterTotal(log) / 1000).toFixed(1)} L`
                            : "—"}
                        </dd>
                      </div>
                      <div>
